...
class SLAM(object):
  ...
  def process_frame(self, img, pose=None, verts=None):
    ...
    #print(np.linalg.inv(f1.pose))   #printing  matrix
...
if __name__ == "__main__":
  ...
  while cap.isOpened():
    ...
    imgIdent +=	1'''
    #print("testing frame print", frame) #this was written  by Chris and not George 
    

    #just prints outthe frame, now to try and write to file
    ###newFile = "./test.txt"
    ##frame.tofile(newFile, sep="|")

    #check out here for resizing, frame is here(below)    
   # print("\n*** frame %d/%d ***" % (i, CNT))
    if ret == True:
